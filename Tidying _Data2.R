...
phyto2022_summary <- phytoplanktoncount2022_data %>%
  ...
  summarise(
    ...
    # PTOX Biovolume Calculation: Sum of biovolume where TARGET_TAXON matches PTOX taxa
    ##To ensure that any species containing the name "Anabaena" for example (including "Anabaena oscillarioides") is captured, we modify the code to use pattern matching with grepl()
...
combined_data5b %>%
  ggplot(aes(x = NTL_RESULT, y = PTL_RESULT)) + #Change variables as many times as possible
  geom_point() +
...
combined_data5b %>%
  filter(!is.na(CHLA_quantile), PH_RESULT > 0, PTL_RESULT > 0) %>% 
  ggplot(aes(x = PH_RESULT, y = PTL_RESULT)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE, color = "red") +
  scale_x_log10() +
  scale_y_log10() +
